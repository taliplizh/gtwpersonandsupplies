    ...
    <center>    
            <div class="block shadow-lg mt-5" style="width: 95%;">
                <div class="block block-rounded block-bordered">
                    <div class="block-header block-header-default">                        
                        <div align="left">
                            <h2 class="block-title" style="font-family:'Kanit',sans-serif;font-size:17px;font-weight:normal;"><B>ประเภทอุบัติการณ์ความเสี่ยง</B></h2>
                        </div> 
                        <div align="left">                       
                        ...
                        @foreach ($risks as $risk)
                            <?php $number++;  ?>
                                <tr height="20">
                                    ...
                                    <td class="text-font text-pedding" width="10%">&nbsp;&nbsp;{{$risk->RISK_GROUPSUBSUB_CODE}}</td>
                                    <td class="text-font text-pedding">&nbsp;&nbsp;{{$risk->RISK_GROUPSUBSUB_NAME}}</td>    
                                    <td class="text-font text-pedding" width="20%">&nbsp;&nbsp;{{$risk->RISK_GROUPSUB_NAME}}</td>    
                                     <td class="text-font text-pedding" width="20%">&nbsp;&nbsp;{{$risk->RISK_GROUP_NAME}}</td>       
                                    <td align="center" width="7%">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu fo13" style="width:10px">      
                                                    <a class="dropdown-item" data-toggle="modal" data-target="#editModal{{$risk->RISK_GROUPSUBSUB_ID}}"><i class="fas fa-edit text-warning mr-2"></i>แก้ไขข้อมูล</a>
                                                    <a class="dropdown-item" href="{{ url('manager_risk/riskgroupsubsub_destroy/'.$risk->RISK_GROUPSUBSUB_ID)}}" onclick="return confirm('ต้องการที่จะยกเลิกการลบข้อมูล ?')"><i class="fas fa-trash text-danger mr-2"></i>ลบข้อมูล</a>
                                                </div>
                                            </div>
                                    </td>                                    
                                </tr>
                                ...
                                <div class="modal fade" id="editModal{{$risk->RISK_GROUPSUBSUB_ID}}" tabindex="-1" aria-labelledby="addModalModalLabel" aria-hidden="true">
                                    <div class="modal-dialog modal-xl">
                                        <div class="modal-content">
                                            <div class="modal-header bg-info shadow-lg">
                                                <h5 class="modal-title text-white" id="addModalModalLabel" style="font-family:'Kanit',sans-serif;font-size:17px;font-weight:normal;">แก้ไขหมวดอุบัติการณ์ความเสี่ยง</h5>
                                            </div>
                                            <div class="modal-body">
                                                <form  method="post" action="{{ route('mrisk.riskgroupsubsub_update') }}" enctype="multipart/form-data">
                                                    @csrf
                                                    <input type="hidden" name="risk_id" id="risk_id" value="{{$risk->RISK_GROUPSUBSUB_ID}}">
                                                    <div class="row text-left">
                                                        <div class="col-md-4">
                                                            <div class="form-group">
                                                                <label for="recipient-name" class="col-form-label">รหัส :</label>
                                                                <input type="text" class="form-control fo13" name="RISK_GROUPSUBSUB_CODE" id="RISK_GROUPSUBSUB_CODE" value="{{$risk->RISK_GROUPSUBSUB_CODE}}" required>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-8">   
                                                            <div class="form-group">
                                                                <label for="message-text" class="col-form-label">ชื่อ :</label>
                                                                <input type="text" class="form-control fo13" name="RISK_GROUPSUBSUB_NAME" id="RISK_GROUPSUBSUB_NAME" value="{{$risk->RISK_GROUPSUBSUB_NAME}}" required>
                                                            </div>
                                                        </div>                                                      
                                                    </div>  
                                                    <div class="row text-left">
                                                                 ...
                                                                 <select name="RISK_GROUP_ID" id="RISK_GROUP_ID" class="form-control fo13" required>
                                                                    <option value="">--เลือก--</option>                           
                                                                        @foreach($riskgroups as $riskgroup) 
                                                                            @if ($risk->RISK_GROUP_ID == $riskgroup->RISK_GROUP_ID)
                                                        ...
                                                        <div class="col-md-6">
                                                            <div class="form-group">
                                                                ...
                                                                <option value="">--เลือก--</option>                           
                                                                    ...
                                                                    @if ($risk->RISK_GROUPSUB_ID == $riskgroupsub->RISK_GROUPSUB_ID)
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <a href="{{ url('manager_risk/riskgroupsubsub')  }}" onclick="return confirm('ต้องการที่จะยกเลิกข้อมูล ?')" class="btn btn-hero-sm btn-hero-danger foo15" onclick="return confirm('ต้องการที่จะยกเลิกการเพิ่มข้อมูล ?')" ><i class="fas fa-window-close mr-2"></i>ยกเลิก</a>
                                            </div>
                                        </div>
                                        </form>
                                    </div>
            ...
            <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-xl">
                    <div class="modal-content">
                        <div class="modal-header bg-info shadow-lg">
                            <h5 class="modal-title text-white" id="addModalModalLabel" style="font-family:'Kanit',sans-serif;font-size:17px;font-weight:normal;">เพิ่มข้อมูลหมวดอุบัติการณ์ความเสี่ยง</h5>
                        </div>
                        <div class="modal-body">
                             <form method="post" action="{{ route('mrisk.riskgroupsubsub_save') }}" enctype="multipart/form-data">
                                @csrf
                                <div class="row text-left">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label for="" class="col-form-label">รหัส :</label>
                                            <input class="form-control fo13" name="RISK_GROUPSUBSUB_CODE" id="RISK_GROUPSUBSUB_CODE" required>
                                        </div>
                                    </div>
                                    <div class="col-md-8">   
                                        <div class="form-group">
                                            <label for="" class="col-form-label">ชื่อ :</label>
                                            <input class="form-control fo13" name="RISK_GROUPSUBSUB_NAME" id="RISK_GROUPSUBSUB_NAME" required>
                                        </div>
                                    </div>                                   
                                </div>
                                 <div class="row text-left">
