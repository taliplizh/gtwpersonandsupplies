...
$pdf->SetFont('THSarabunNew','',15);
$pdf->WriteHTML(iconv( 'UTF-8','cp874' ,'ลาคลอดบุตร'));
...
$pdf-> Image(base_path('public').'/fpdf/img/checkno.jpg',55,62,4,4);
$pdf-> Image(base_path('public').'/fpdf/img/checkno.jpg',55,70,4.5,4.5);
$pdf-> Image(base_path('public').'/fpdf/img/checked.png',55,78,4.5,4.5);
...
$detail .= $inforleave->LEAVE_CONTACT.' โทรศัพท์ '.$inforleave->LEAVE_CONTACT_PHONE.'<br>';
$pdf->WriteHTML(iconv( 'UTF-8','cp874' ,$detail));
}else{

  if($lastspawn->LEAVE_DATE_SUM == 0.5){ 
    $dateresult = 'ครึ่ง';
  }else{
    $dateresult = number_format($lastspawn->LEAVE_DATE_SUM);
  }

  $detail ='ตั้งแต่วันที่  '.dayThai($inforleave->LEAVE_DATE_BEGIN).'  เดือน    '.monthThai($inforleave->LEAVE_DATE_BEGIN).'    พ.ศ. '.yearThai($inforleave->LEAVE_DATE_BEGIN).'    ถึงวันที่   '.dayThai($inforleave->LEAVE_DATE_END).'  เดือน   '.monthThai($inforleave->LEAVE_DATE_END).'     พ.ศ. '.yearThai($inforleave->LEAVE_DATE_END).'    มีกำหนด   '.$dateresultwork.'   วัน<br>';
  $detail .='ข้าพเจ้าได้ลา            ป่วย            กิจส่วนตัว          คลอด    ครั้งสุดท้าย    ตั้งแต่วันที่   '.dayThai($lastspawn->LEAVE_DATE_BEGIN).'  เดือน   '.monthThai($lastspawn->LEAVE_DATE_BEGIN).'     พ.ศ. '.yearThai($lastspawn->LEAVE_DATE_BEGIN).'<br>';
  $detail .='ถึงวันที่    '.dayThai($lastspawn->LEAVE_DATE_END).'  เดือน   '.monthThai($lastspawn->LEAVE_DATE_END).'  พ.ศ. '.yearThai($lastspawn->LEAVE_DATE_END).'   มีกำหนด     '.$dateresult.'     วัน   ในระหว่างลาติดต่อข้าพเจ้าได้ที่<br>';
  $detail .= $inforleave->LEAVE_CONTACT.' โทรศัพท์ '.$inforleave->LEAVE_CONTACT_PHONE.'<br>';
  $pdf->WriteHTML(iconv( 'UTF-8','cp874' ,$detail));

}
$pdf-> Image(base_path('public').'/fpdf/img/checkno.jpg',47,90.5,4,4);
$pdf-> Image(base_path('public').'/fpdf/img/checkno.jpg',65,90.5,4.5,4.5);
$pdf-> Image(base_path('public').'/fpdf/img/checked.png',91,90.5,4.5,4.5);
...
if($checksig == 1 && $sig1 !== null && $sig1 !== ''){
  $pdf-> Image(Storage::path('public/images/'.$sig1),140,109.5,20,10);
  }
$pdf->WriteHTML(iconv( 'UTF-8','cp874' ,'
...
$total = $sumbirth + $inforleave->WORK_DO;
$pdf->SetFont('THSarabunNew','',15);
$table_b ='<tr><td width="70" height="50" >ป่วย</td>
<td width="70" height="50" >'.number_format($sumsick, 1, '.', '').'</td>
<td width="70" height="50" >0.00</td>
<td width="70" height="50" >'.number_format($sumsick, 1, '.', '').'</td></tr>
...
<td width="70" height="50" >'.number_format($total, 1, '.', '').'</td></tr></table>';


if($checksig == 1 && $sig5 !== null && $sig5 !== ''){
  $pdf-> Image(Storage::path('public/images/'.$sig5),35,168,20,10);
  }
...
if($checksig == 1 && $sig6 !== null && $sig6 !== ''){
  $pdf-> Image(Storage::path('public/images/'.$sig6),35,208.5,20,10);
  }
...
if($checksig == 1 && $sig7 !== null && $sig7 !== ''){
  $pdf-> Image(Storage::path('public/images/'.$sig7),35,238.5,20,10);
  }
