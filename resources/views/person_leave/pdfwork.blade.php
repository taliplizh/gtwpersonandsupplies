...
$pdf->SetFont('THSarabunNew','',15);
$pdf->WriteHTML(iconv( 'UTF-8','cp874' ,'ลากิจส่วนตัว'));
...
$pdf->Image(base_path('public').'/fpdf/img/checked.png',55,70,4.5,4.5);
...
$pdf->WriteHTML(iconv( 'UTF-8','cp874' ,$detail));
  ...
  if($lastswork->LEAVE_DATE_SUM == 0.5){ 
    $dateresult = 'ครึ่ง';
  }else{
    $dateresult = number_format($lastswork->LEAVE_DATE_SUM);
  }

  $detail ='ตั้งแต่วันที่  '.dayThai($inforleave->LEAVE_DATE_BEGIN).'  เดือน    '.monthThai($inforleave->LEAVE_DATE_BEGIN).'    พ.ศ. '.yearThai($inforleave->LEAVE_DATE_BEGIN).'    ถึงวันที่   '.dayThai($inforleave->LEAVE_DATE_END).'  เดือน   '.monthThai($inforleave->LEAVE_DATE_END).'     พ.ศ. '.yearThai($inforleave->LEAVE_DATE_END).'    มีกำหนด   '.$dateresultwork.'   วัน<br>';
  $detail .='ข้าพเจ้าได้ลา            ป่วย            กิจส่วนตัว          คลอด    ครั้งสุดท้าย    ตั้งแต่วันที่   '.dayThai($lastswork->LEAVE_DATE_BEGIN).'  เดือน   '.monthThai($lastswork->LEAVE_DATE_BEGIN).'     พ.ศ. '.yearThai($lastswork->LEAVE_DATE_BEGIN).'<br>';
  $detail .='ถึงวันที่    '.dayThai($lastswork->LEAVE_DATE_END).'  เดือน   '.monthThai($lastswork->LEAVE_DATE_END).'  พ.ศ. '.yearThai($lastswork->LEAVE_DATE_END).'   มีกำหนด     '.$dateresult.'     วัน   ในระหว่างลาติดต่อข้าพเจ้าได้ที่<br>';
...
$pdf->Image(base_path('public').'/fpdf/img/checked.png',65,90.5,4.5,4.5);
$pdf->Image(base_path('public').'/fpdf/img/checkno.jpg',91,90.5,4.5,4.5);

$pdf->SetFont('THSarabunNew Bold','',15);

if($checksig == 1 && $sig1 !== null && $sig1 !== ''){
  $pdf->Image(Storage::path('public/images/'.$sig1),140,120.5,20,10);
  }
...
$total = $sumwork + $inforleave->WORK_DO;
$pdf->SetFont('THSarabunNew','',15);
