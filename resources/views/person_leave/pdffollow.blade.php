...
$data1.= "<br>
 ...
 $pdf->SetFont('THSarabunNew','',15);
 $pdf->WriteHTML(iconv( 'UTF-8','cp874' ,'ลาติดตามคู่สมรส'));
...
$detail ='           มีความประสงค์ลาไปติดตามคู่สมรสโดยชอบด้วยกฎหมายชื่อ '.$inforleave->FW_MARRY.' ตำแหน่ง '.$inforleave->FW_MARRY_POSITION.' จึงขออนุญาตติดตามคู่สมรส<br>';
...
$pdf->Text(103,168,iconv( 'UTF-8','TIS-620','('.$inforleave->LEADER_PERSON_NAME.')'));
$pdf->Text(97,177,iconv( 'UTF-8','TIS-620','ตำแหน่ง '.$inforleave->LEADER_POSITION_IN_WORK));
$pdf->Text(100,186,iconv( 'UTF-8','TIS-620','วันที่  '.formate($inforleave->updated_at)));
...
if($inforleave->LEAVE_STATUS_CODE == 'Allow'){ 
  ...
  $pdf-> Image(base_path('public').'/fpdf/img/checkno.jpg',57,205,4.5,4.5);
}else{
  $pdf-> Image(base_path('public').'/fpdf/img/checkno.jpg',30,205,4.5,4.5);
  $pdf-> Image(base_path('public').'/fpdf/img/checkno.png',57,205,4.5,4.5);
}
...
$pdf->Text(103,256,iconv( 'UTF-8','TIS-620','( '.$orgname->HR_PREFIX_NAME.''.$orgname->HR_FNAME.' '.$orgname->HR_LNAME.' )'));
$pdf->Text(97,266,iconv( 'UTF-8','TIS-620','ผู้อำนวยการ'.$orgname->ORG_NAME));
$pdf->Text(100,276,iconv( 'UTF-8','TIS-620','วันที่ '.formate($inforleave->updated_at)));
