...
<div class="block block-rounded block-bordered" style="width: 95%;margin:auto">
    <div class="block-header block-header-default">
        <h3 class="block-title text-center fw-8">แก้ไขข้อมูลการตรวจสอบเครื่องนึ่ง/อบ</h3>
    </div>
    <div class="block-content">
                <form action="{{route('mpay.mpay_maintenance_machine_update')}}" method="POST">
                    <input type="hidden" name="MMAINTENANCE_ID" value="{{$maintenance->MMAINTENANCE_ID}}">
                @csrf
                    <!-- Basic Elements -->
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <select class="js-select2 form-control" id="CPAY_MACH" name="CPAY_MACH"
                                    ...
                                    ?>
                                    @if($maintenance->CPAY_MACH_ID === $row->CPAY_MACH_ID)
                                    <option value="{{$data_machine_record}}" data-record="{{$data_machine_record}}" selected>{{$row->CPAY_MACH_NAME_INSIDE}} ({{$row->CPAY_MACH_NUMBER}})</option>
                                    @else
                            ...
                            <div class="form-group">
                                ...
                                <select class="js-select2 form-control" id="MMAINTENANCE_RESULT" name="MMAINTENANCE_RESULT"
                                    ...
                                    <option></option>
                                    @if($maintenance->MMAINTENANCE_RESULT === 1)
                                    <option value="1" selected>ผ่าน</option>
                                    <option value="2">ไม่ผ่าน</option>
                                    @else
                                    <option value="1">ผ่าน</option>
                                    <option value="2" selected>ไม่ผ่าน</option>
                                    @endif
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="js-select2 form-control" id="CHECK_MACHINE_PERSON" name="CHECK_MACHINE_PERSON"
                                    ...
                                    ?>
                                     @if($maintenance->CHECK_MACHINE_PERSON_ID === $row->ID)
                                    <option value="{{$data_check_machine_record}}" data-record="{{$data_check_machine_record}}" selected>{{$row->HR_FNAME}} {{$row->HR_LNAME}}</option>
                                     @else
                            ...
                            <div class="form-group">
                                ...
                                <select class="js-select2 form-control" id="CHECK_PERSON" name="CHECK_PERSON"
                                    ...
                                    ?>
                                     @if($maintenance->CHECK_PERSON_ID === $row->ID)
                                    <option value="{{$data_check_record}}" data-record="{{$data_check_record}}" selected>{{$row->HR_FNAME}} {{$row->HR_LNAME}}</option>
                                     @else
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <input  name="MMAINTENANCE_TEST_DATE"  id="MMAINTENANCE_TEST_DATE" class="form-control input-lg datepicker f-kanit" data-date-format="mm/dd/yyyy"  value="{{$maintenance->MMAINTENANCE_TEST_DATE}}" required readonly>
                            </div>
                            <div class="form-group">
                                <label for="MMAINTENANCE_TEST_TIME">เวลาตรวจเครื่องนึ่ง/อบ</label> <span class="text-danger">*</span>
                                <input  name="MMAINTENANCE_TEST_TIME"  id="MMAINTENANCE_TEST_TIME" type="time" class="form-control input-lg f-kanit" value="{{$maintenance->MMAINTENANCE_TEST_TIME}}" required>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-lg-12">
                            <div class="form-group">
                                ...
                                <textarea type="text" style="height:90px;"
                                    ...
                                    name="MMAINTENANCE_DETAIL" maxlength="400" placeholder="รายละเอียด...">{{$maintenance->MMAINTENANCE_DETAIL}}</textarea>
                            </div>
                        </div>
                    </div>
                    <div class="row items-push">
