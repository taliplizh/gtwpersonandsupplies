...
?>
                ...
                <h2 class="content-heading pt-0" style="font-family: 'Kanit', sans-serif;">แก้ไขข้อมูลเชื้อเพลิง</h2>    

    
        <form  method="post" action="{{ route('admin.updatecarpower') }}" enctype="multipart/form-data">
        @csrf
        <div class="row push">
    ...
    <div class="col-lg-2">
      ...
      <div class="col-lg-3">
      <input  name = "CAR_POWER_ID_NAME"  id="CAR_POWER_ID_NAME" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" value="{{$infocarpower->CAR_POWER_ID_NAME}}" onkeyup="check_car_power_id_name();">
      ...
      <div class="col-lg-3">
      <input  name = "CAR_POWER_NAME"  id="CAR_POWER_NAME" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" value="{{$infocarpower->CAR_POWER_NAME}}" onkeyup="check_car_power_name();" >
      <div style="color: red; font-size: 16px;" id="car_power_name"></div>  
    </div>

      <input  type="hidden" name = "CAR_POWER_ID"  id="CAR_POWER_ID" class="form-control input-lg" value="{{$infocarpower->CAR_POWER_ID}}">
      </div></div>
        <div class="modal-footer">
        <div align="right">
        <button type="submit"  class="btn btn-hero-sm btn-hero-info" >บันทึกแก้ไขข้อมูล</button>
         <a href="{{ url('admin_car/setupcarpower')  }}" class="btn btn-hero-sm btn-hero-danger" onclick="return confirm('ต้องการที่จะยกเลิกการแก้ไขข้อมูล ?')" >ยกเลิก</a> 
         </div>    
