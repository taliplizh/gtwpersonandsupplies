  ...
  <div class="block block-rounded block-bordered">
    <div class="block-header">
      <div class="block-title">
        กำหนดสิทธิ์การประเมิน KPI
      </div>
      <div class="block-options">
        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#ModalCenter">
          <i class="fas fa-plus mr-2"></i>เพิ่มข้อมูลผู้ประเมิน
        </button>
      </div>
    </div>
        ...
        <table class="gwt-table table-striped table-vcenter" width="100%">
          <thead style="background-color: #FFEBCD;">
            <tr height="40">
              ...
              <th class="text-font" style="border-color:#F0FFFF;text-align: center;" width="8%">ผู้ถูกประเมิน</th>
          ...
          <tbody>
            ...
            <tr class="p-5px">
              ...
              <td class="text-center">
                <a href="{{route('mperson.permission_job_person',$row->IWJOB_PERMIS_ID)}}" class="btn btn-success"><i
                    class=" fa fa-user-friends"></i></a>
              </td>
              <td class="text-center">
                <a href="{{route('mperson.permission_job.delete',$row->IWJOB_PERMIS_ID)}}" class="btn btn-danger"
                  onclick="return confirm('ต้องการที่จะลบข้อมูล {{$row->HR_FNAME}}   {{$row->HR_LNAME}}?')"><i
...
<div class="modal fade" id="ModalCenter" role="dialog" aria-labelledby="ModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <form action="{{route('mperson.permission_job_save')}}" method="post">
        @csrf
