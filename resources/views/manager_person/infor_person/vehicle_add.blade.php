...
?>
        ...
        <form  method="post" action="{{ route('mperson.vehicle_save') }}" enctype="multipart/form-data">
        @csrf
        <div class="row push">
        <div class="col-lg-4">
    ...
    <div class="col-lg-4">
      ...
      <input type="hidden" name = "USER_EDIT_ID"  id="USER_EDIT_ID" value="{{ $id_user }} ">
        ...
        <button type="submit"  class="btn btn-hero-sm btn-hero-info" ><i class="fas fa-save"></i> &nbsp;บันทึกข้อมูล</button>
         <a href="{{ url('manager_person/inforperson/view/vehicle/'.$inforpersonuserofficialid -> ID)  }}" class="btn btn-hero-sm btn-hero-danger" onclick="return confirm('ต้องการที่จะยกเลิกการแก้ไขข้อมูล ?')" ><i class="fas fa-window-close"></i> &nbsp;ยกเลิก</a> 
         </div>    
