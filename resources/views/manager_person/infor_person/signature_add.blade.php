...
?>
                    ...
                    <div class="content content-full">
                        <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                            <h1 style="font-family: 'Kanit', sans-serif; font-size:15px;font-size: 1.3rem;font-weight:normal;">{{ $inforpersonusersignature -> HR_PREFIX_NAME }}   {{ $inforpersonusersignature -> HR_FNAME }}  {{ $inforpersonusersignature -> HR_LNAME }}</h1>
        ...
        <form  method="post" action="{{ route('mperson.signature_save') }}" enctype="multipart/form-data">
        @csrf
        <div class="row push">
        <div class="col-lg-4">
      ...
      <input type="hidden" name = "USER_EDIT_ID"  id="USER_EDIT_ID" value="{{ $id_user }} ">
        ...
        <button type="submit"  class="btn btn-hero-sm btn-hero-info" ><i class="fas fa-save"></i> &nbsp;บันทึกข้อมูล</button>
         <a href="{{ url('manager_person/inforperson/view/signature/'.$inforpersonuserid -> ID)  }}" class="btn btn-hero-sm btn-hero-danger" onclick="return confirm('ต้องการที่จะยกเลิกการเพิ่มข้อมูล ?')" ><i class="fas fa-window-close"></i> &nbsp;ยกเลิก</a> 
         </div>    
