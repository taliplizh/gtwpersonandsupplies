...
?>
                ...
                <h2 class="content-heading pt-0" style="font-family: 'Kanit', sans-serif;">แก้ไขรายการคลังพัสดุ</h2>    

    
        <form  method="post" action="{{ route('admin.updatesuppliesinven') }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="row push">                       
            ...
            <div class="col-lg-3">
            <input  name = "INVEN_NAME"  id="INVEN_NAME" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" value="{{ $infosuppliesinvenT -> INVEN_NAME }}" onkeyup="check_invenname();">
            <div style="color: red; font-size: 13px;" id="invenname"></div>
        </div>
            <div class="col-lg-1">
                    <label >ชื่อย่อ</label>
            </div>
            <div class="col-lg-3">
                <input  name = "INVEN_NAME_SHORT"  id="INVEN_NAME_SHORT" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" value="{{ $infosuppliesinvenT -> INVEN_NAME_SHORT }}" onkeyup="check_invennameshort();">
                <div style="color: red; font-size: 13px;" id="invennameshort"></div> 
            </div>                    
            <div class="col-sm-1">
                <label >ผู้รับผิดชอบหลัก</label>
            </div>
            <div class="col-sm-3">
                    <select name="INVEN_HR_ID" id="INVEN_HR_ID" class="form-control input-lg js-example-basic-single" style=" font-family: 'Kanit', sans-serif;font-size: 13px;" onchange="check_invenhrid();">
                            <option value="">--เลือกผู้รับผิดชอบหลัก--</option>
                                 @foreach ($personuserT as $personuser) 
                                    @if($infosuppliesinvenT -> INVEN_HR_ID == $personuser-> ID )                                                     
                    ...
                    <div style="color: red; font-size: 13px;" id="invenhrid"></div>                         
                </div>                        
            </div>
            <div class="row push">                       
                <div class="col-lg-1">
                ...
                <div class="col-lg-3">
                    <select name="INVEN_LOCATION_ID" id="INVEN_LOCATION_ID" class="form-control input-lg js-example-basic-single" style=" font-family: 'Kanit', sans-serif;font-size: 13px;" onchange="check_invenlocationid();">
                        <option value="">--เลือกสถานที่ตั้ง--</option>
                            @foreach ($locationT as $location) 
                                @if($infosuppliesinvenT -> INVEN_LOCATION_ID == $location-> LOCATION_ID )                                                    
                    ...
                    <div style="color: red; font-size: 13px;" id="invenlocationid"></div> 
                </div>                      
                <div class="col-sm-1">
                    <label >ผู้ออกใบสั่งซื้อ</label>
                </div>
                <div class="col-sm-3">
                    <select name="INVEN_ORDER_HR_ID" id="INVEN_ORDER_HR_ID" class="form-control input-lg js-example-basic-single" style=" font-family: 'Kanit', sans-serif;font-size: 13px;" onchange="check_invenorderhrid();">
                            <option value="">--เลือกผู้ออกใบสั่งซื้อ--</option>
                             @foreach ($personuserT as $personuser) 
                            @if($infosuppliesinvenT -> INVEN_ORDER_HR_ID == $personuser-> ID )                                                     
                                <option value="{{$personuser-> ID }}" selected>{{ $personuser-> HR_FNAME }} {{ $personuser-> HR_LNAME }}</option>                                            
                            @else
                                <option value="{{$personuser-> ID }}" >{{ $personuser-> HR_FNAME }} {{ $personuser-> HR_LNAME }}</option>
                            @endif
                    ...
                    <div style="color: red; font-size: 13px;" id="invenorderhrid"></div> 
                </div>
                <div class="col-sm-1">
                    <label >ผู้ขอซื้อ</label>
                </div>
                <div class="col-sm-3">
                    <select name="INVEN_WRITE_HR_ID" id="INVEN_WRITE_HR_ID" class="form-control input-lg js-example-basic-single" style=" font-family: 'Kanit', sans-serif;font-size: 13px;" onchange="check_invenwritehrid();">
                            <option value="">--เลือกผู้ขอซื้อ--</option>
                             @foreach ($personuserT as $personuser) 
                            @if($infosuppliesinvenT -> INVEN_WRITE_HR_ID == $personuser-> ID )                                                     
                                <option value="{{$personuser-> ID }}" selected>{{ $personuser-> HR_FNAME }} {{ $personuser-> HR_LNAME }}</option>                                            
                            @else
                                <option value="{{$personuser-> ID }}" >{{ $personuser-> HR_FNAME }} {{ $personuser-> HR_LNAME }}</option>
                            @endif
                    ...
                    <div style="color: red; font-size: 13px;" id="invenwritehrid"></div>  
                </div>
                </div>
                <div class="row push">                       
                    <div class="col-sm-1">
                    ...
                    <div class="col-sm-3">
                        <select name="INVEN_BUY_HR_ID" id="INVEN_BUY_HR_ID" class="form-control input-lg js-example-basic-single" style=" font-family: 'Kanit', sans-serif;font-size: 13px;" onchange="check_invenbuyhrid();">
                            <option value="">--เลือกผู้อนุมัติซื้อ--</option>
                             @foreach ($personuserT as $personuser) 
                            @if($infosuppliesinvenT -> INVEN_BUY_HR_ID == $personuser-> ID )                                                     
                                <option value="{{$personuser-> ID }}" selected>{{ $personuser-> HR_FNAME }} {{ $personuser-> HR_LNAME }}</option>                                            
                            @else
                                <option value="{{$personuser-> ID }}" >{{ $personuser-> HR_FNAME }} {{ $personuser-> HR_LNAME }}</option>
                            @endif
                        ...
                        <div style="color: red; font-size: 13px;" id="invenbuyhrid"></div> 
                    </div>
                <div class="col-sm-1">
                    <label >ตำแหน่งผู้อนุมัติซื้อ</label>
                </div>
                <div class="col-sm-3">
                    <select name="INVEN_BUY_POSITION" id="INVEN_BUY_POSITION" class="form-control input-lg js-example-basic-single" style=" font-family: 'Kanit', sans-serif;font-size: 13px;" onchange="check_invenbuyposition();" >
                            <option value="">--เลือกตำแหน่งผู้อนุมัติซื้อ--</option>
                             @foreach ($positions as $position) 
                                    @if($infosuppliesinvenT -> INVEN_BUY_POSITION == $position-> HR_POSITION_ID )                                                     
                                        <option value="{{$position-> HR_POSITION_ID }}" selected>{{ $position-> HR_POSITION_NAME }}</option>                                            
                                    @else
                                        <option value="{{$position-> HR_POSITION_ID }}" >{{ $position-> HR_POSITION_NAME }}</option>
                                    @endif
                                @endforeach  
                    </select>
                    <div style="color: red; font-size: 13px;" id="invenbuyposition"></div>  
                </div>
                <input value="{{$infosuppliesinvenT-> INVEN_ID}}" type="hidden" name = "INVEN_ID"  id="INVEN_ID" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" >       
                </div> 

        <div class="modal-footer">
        <div align="right">
        <button type="submit"  class="btn btn-hero-sm btn-hero-info" ><i class="fas fa-save mr-2"></i>บันทึกแก้ไขข้อมูล</button>
         <a href="{{ url('admin_warehouse/setupsuppliesinven')  }}" class="btn btn-hero-sm btn-hero-danger" onclick="return confirm('ต้องการที่จะยกเลิกการแก้ไขข้อมูล ?')" ><i class="fas fa-window-close mr-2"></i>ยกเลิก</a> 
         </div>    
